  ...
  in {
    ...
    nixosModules.nordlayer = {
      ...
      options.services.nordlayer.enable =
        lib.mkEnableOption "NordLayer integration (package + nix-ld)";

      options.services.nordlayer.extraNixLdLibraries = lib.mkOption {
        type = lib.types.listOf lib.types.package;
        default = with pkgs; [libcap_ng];
      ...
      config = lib.mkIf config.services.nordlayer.enable {
        environment.systemPackages = [self.packages.${pkgs.stdenv.hostPlatform.system}.nordlayer];
